...
stg.TC,
case when F.OFFER_STS_ID = 1 AND F.USED_CNT = 0  then 'ACTIVE '||dbm.segment else 'REDEEMED' end as COUPON,
F.OFFER_CD as COUPON_CODE,
END_TS
 FROM p_dbm_input_t.adobe_de_familymen_mam stg
 INNER JOIN unica_temp.DE_FM_SS_20191212_MAM mam on stg.user_id=mam.user_id
...
INNER JOIN P_MDS_T.DE_SpendStretch_Dec_DBM DBM 
     ON         stg.user_id=dbm.primary_user_id
WHERE 7=7 and end_ts>current_date
-- CHANGE MANHATTAN ID'S
 AND        F.LYLTY_CMPGN_ID IN ( 6342767502) 
          --AND        F.LYLTY_CMPGN_ID IN (6342746701 )  --6342733801
AND mam.segment not like 'SEED%'
...
'FAMILY_MEN' as SEGMENT,
'T' as TC,
case when F.OFFER_STS_ID = 1 AND F.USED_CNT = 0  then 'ACTIVE Generic' else 'REDEEMED' end as COUPON,
F.OFFER_CD as COUPON_CODE,
END_TS
 FROM unica_temp.DE_FM_SS_20191212_MAM mam 
 INNER JOIN PRS_RESTRICTED_V.EIP_LYLTY_OFFER F 
          ON     F.USER_ID=mam.USER_ID
  INNER JOIN  PRS_RESTRICTED_V.EIP_LYLTY_CMPGN C 
          ON         F.LYLTY_CMPGN_ID = C.LYLTY_CMPGN_ID 
   INNER JOIN   PRS_RESTRICTED_V.EIP_CPN_DTL D 
          ON         D.INCNTV_CD = C.CMPGN_NAME 
...
current_date as END_TS
 FROM p_dbm_input_t.adobe_de_familymen_mam stg
 where user_id not in (select user_id from  PRS_RESTRICTED_V.EIP_LYLTY_OFFER F 
  INNER JOIN PRS_RESTRICTED_V.EIP_LYLTY_CMPGN C 
          ON         F.LYLTY_CMPGN_ID = C.LYLTY_CMPGN_ID 
   INNER JOIN  PRS_RESTRICTED_V.EIP_CPN_DTL D 
          ON         D.INCNTV_CD = C.CMPGN_NAME 
...
AND hist.ebypls_buyer_status_cd = -1
...
FROM
	UNICA_TEMP.DE_FAMILY_MEN_DAILY AS AUD
		...
		INNER JOIN UNICA_TEMP.XMP_MESSAGE AS MSG
			ON	MSG.MSG_ID = MAP_SM.MSG_ID AND
					CURRENT_DATE BETWEEN MSG.START_DATE-10 AND MSG.END_DATE AND
					MSG.MSG_NAME = 'DE_FM014_SPEND_STRETCH'	
	WHERE 7=7
	and TC='T'
	AND COUPON='ACTIVE'
	AND COUPON_CODE = 'MEHRRABATT10';
...
FROM
	UNICA_TEMP.DE_FAMILY_MEN_DAILY AS AUD
		...
		INNER JOIN UNICA_TEMP.XMP_MESSAGE AS MSG
			ON	MSG.MSG_ID = MAP_SM.MSG_ID AND
					CURRENT_DATE BETWEEN MSG.START_DATE-10 AND MSG.END_DATE AND
					MSG.MSG_NAME = 'DE_FM015_SPEND_STRETCH'	
	WHERE 7=7
	and TC='T'
	AND COUPON='ACTIVE'
	AND COUPON_CODE = 'MEHRRABATT15';
	*/
...
/* PREPARING DATA FOR  SENDOUT - 5th */
